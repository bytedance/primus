...
public class SuspendStatusServlet extends HttpServlet {

  private static final Logger LOG = LoggerFactory.getLogger(SuspendStatusServlet.class);
  private static AMContext context;

  public static void setContext(AMContext context) {
    SuspendStatusServlet.context = context;
  }

  @Override
  protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws IOException {
    String path = req.getRequestURI().substring(req.getContextPath().length());
    LOG.info("receive HTTP getSuspendStatus request:" + path);
