...
public class HsView extends TwoColumnLayout {
   ...
   */
  protected void preHead(Page.HTML html) {
    commonPreHead(html);
   ...
   */
  protected void commonPreHead(Page.HTML html) {
  ...
  /**
   * @return The end of a javascript map that is the jquery datatable configuration for the jobs
   * table.  the Jobs table is assumed to be rendered by the class returned from {@link #content()}
   */
  private String jobsTableInit() {
    return tableInit().
        ...
        // Sort by start time desc
            append(", aaSorting: [[1, 'desc']]").
        append("}").
        toString();
  }

  /**
   * @return javascript to add into the jquery block after the table has been initialized. This code
   * adds in per field filtering.
   */
