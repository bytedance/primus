...
public class GroupByKeyImpl<T extends Input> implements GroupByKey<T> {
  ...
  public List<Pair<String, List<T>>> apply(List<Pair<String, T>> inputs) {
    ...
    for (Pair<String, T> pair : inputs) {
      List<T> list = tmpMap.get(pair.getKey());
      if (list == null) {
        list = new LinkedList<>();
        tmpMap.put(pair.getKey(), list);
      }
      list.add(pair.getValue());
    }

    return tmpMap.entrySet().stream().map(e -> new Pair<>(e.getKey(), e.getValue()))
