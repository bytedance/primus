...
import com.bytedance.primus.common.model.records.impl.pb.ValueRangesPBImpl;
...
public abstract class ValueRanges implements Comparable<ValueRanges> {
  ...
  public static ValueRanges newInstance(List<ValueRange> rangesList) {
    ValueRanges valueRanges = new ValueRangesPBImpl();
  ...
  /**
   * @param uranges that may be ValueRanges or BitSetStores, if it's BitSetStores, do nothing
   * @return ValueRanges that is coalesced
   */
  public static ValueRanges coalesce(ValueRanges uranges) {
    if (uranges == null) {
      return null;
    }
  ...
  /**
   * This method is used to check if the ValueRanges coalesced, coalesced means no override parts
   * and well sorted. For example, [1-3],[5-10] is coalesced, and [1-4],[3-10] and [5-10].[1-3] is
   * not.
   *
  ...
  /**
   * Initialize the ValueRanges from expression, we current support[1-3],[5-10] style
   *
   * @param expression
   * @return
   */
  ...
  /**
   * Initialize the ValueRanges from expression, we currently support[1-3],[5-10] style
   *
   * @param expression
   * @return ValueRanges
   */
