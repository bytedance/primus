...
public class Data extends AbstractApiType<Data, DataSpec, DataStatus> {
  ...
  @Override
  public Data setSpec(DataSpec spec) {
    this.spec = spec;
    return this;
  }
  ...
  @Override
  public Data setStatus(DataStatus status) {
    this.status = status;
    return this;
  }
  ...
  @Override
  public Data fromResource(Resource resource) throws ApiServerException {
    if (resource.getKind().equals(this.KIND)) {
      try {
        meta = resource.getMeta();
        spec = new DataSpecImpl(resource.getSpec().unpack(DataProto.DataSpec.class));
        // ensure status has been set
        if (resource.getStatus().is(DataProto.DataStatus.class)) {
          status = new DataStatusImpl(resource.getStatus().unpack(DataProto.DataStatus.class));
        } else {
          status = new DataStatusImpl();
        }
        return this;
