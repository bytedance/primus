...
public interface ApiType<T extends ApiType, TSpec, TStatus> {
    String getKind();
    Meta getMeta();
    T setMeta(Meta meta);
    TSpec getSpec();
    T setSpec(TSpec spec);
    TStatus getStatus();
    T setStatus(TStatus status);
    Resource toResource();
    T fromResource(Resource resource) throws ApiServerException;
}
